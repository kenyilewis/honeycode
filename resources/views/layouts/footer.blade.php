<footer class="py-5 footer-main">
  <div class="container">
    <p class="m-0 text-center text-white">Copyright HoneyCode</p>
  </div>
  <p class="text-center"> <a href="http://facebook.com" class="text-white" target="_blank" ><i class="fab fa-facebook-square social" ></i></a>
  <a href="http://instagram.com" class="text-white" target="_blank" ><i class="fab fa-instagram social" ></i></a>
  <a href="http://twitter.com" class="text-white" target="_blank" ><i class="fab fa-twitter-square social" ></i></a></p>
</footer>

<!-- Bootstrap core JavaScript -->
<script src="vendor/jquery/jquery.min.js"></script>
<script src="vendor/bootstrap/js/bootstrap.bundle.min.js"></script>
</div>
